...
def ophthalmic_procedure(
    ...
    crosses = [
        [x, y]
        for x in [-screen_res["width"] / 2 + vis_offset, screen_res["width"] / 2 - vis_offset]
        for y in [-screen_res["height"] / 2 + vis_offset, screen_res["height"] / 2 - vis_offset]
    ]
    crosses = [
        visual.TextStim(win=win, text="+", height=3 * text_size, color="black", pos=pos)
        for pos in crosses
    ]
    ...
    for frame_counter in range(frames_per_sec * secs_of_blinks):
        if frame_counter % frames_per_sec == 0:
            trigger_no, triggers_list = prepare_trigger(
                trigger_type=TriggerTypes.BLINK, trigger_no=trigger_no, triggers_list=triggers_list
            )
            send_trigger(
                port_eeg=port_eeg,
                port_nirs=port_nirs,
