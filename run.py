...
def run():
    ...
    # prepare training
    train_data, exp_data = prepare_words(win=win, folder_name="stops", config=config, screen_res=screen_res,
                                         experiment_version=experiment_version)
    arrows = load_data(win=win, folder_name="arrows", config=config, screen_res=screen_res)

    training_block_1 = prepare_trials(number_of_blocks=1, stops=train_data, arrows=arrows,
    ...
    training_block_2 = prepare_trials(number_of_blocks=1, stops=train_data, arrows=arrows,
    ...
    experiment_block = prepare_trials(number_of_blocks=config['Number_of_experiment_blocks'],
                                      ...
                                      percent_of_trials_with_stop=config['Percent_of_trials_with_stop'],
                                      stops=exp_data, arrows=arrows, messages=breaks)

    stops_times = create_stops_times_dict(stops=exp_data, start_wait_to_stop=config['Start_wait_to_stop'])
    ...
    for instruction in instructions:
        show_info(win=win, file_name=os.path.join('messages', instruction), text_size=config['Text_size'],
                  screen_width=screen_res['width'], insert=buttons_info, triggers_list=[], part_name=part_name, data=[])

    # Training
    beh, _ = show(config=config, win=win, screen_res=screen_res, frames_per_sec=frames_per_sec,
                  blocks=training_block, stops_times=stops_times_train, trigger_no=trigger_no,
                  triggers_list=[], part_id=part_name, data=[])
    ...
    beh, triggers_list = show(config=config, win=win, screen_res=screen_res, frames_per_sec=frames_per_sec,
                              ...
                              trigger_no=trigger_no, triggers_list=triggers_list, part_id=part_name, data=beh)
